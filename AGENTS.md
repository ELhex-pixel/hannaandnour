...
- `/api/products`, `/api/products?slug=`, `/api/checkout`, `/api/orders?session_id=` or `?email=`, `/api/auth` (client accounts: actions `signup`/`login`/`refresh`/`me`/`orders`/`wishlist`, sessions = Supabase Auth access+refresh tokens, all validated server-side via `requireUser` in `shared.js`), `/api/newsletter`, `/api/reviews`, `/api/contact` (contact form → `contact_messages` table), `/api/stripe-webhook`, `/api/config` (public settings), `/api/admin/*` (authed) (all proxied from `/.netlify/functions/*` via netlify.toml).
- Client accounts (Phase 2): signup/login in `netlify/functions/auth.js` (service-role client, `email_confirm: true`, orders are linked to `orders.user_id` by email on login). The account page shows the auth panel, an account-panel with orders once signed in, and keeps the "enter your email" lookup for guests. When signed in, `checkout.js` sends `auth_token` so the new order is attached to the account; `auth.js` merges the local wishlist with the account's on login and backs up heart toggles (debounced). RLS for direct anon access: `supabase/rls_accounts.sql` (app works via service role either way).
